...
class ResUsers(models.Model):
    ...
    password_history_ids = fields.One2many(string='Password History', comodel_name='res.users.pass.history', inverse_name='user_id', readonly=True)
    ...
    def password_match_message(self):
        ...
        if company_id.password_lower:
            message.append(_('* Lowercase letter (At least {} character)').format(company_id.password_lower))
        if company_id.password_upper:
            message.append(_('* Uppercase letter (At least {} character)').format(company_id.password_upper))
        if company_id.password_numeric:
            message.append(_('* Numeric digit (At least {} character)').format(company_id.password_numeric))
        if company_id.password_special:
            message.append(_('* Special character (At least {} character)').format(company_id.password_special))
        if message:
            message = [_('Must contain the following:')] + message
        if company_id.password_length:
            message = [_('Password must be {} characters or more.').format(company_id.password_length)] + message
        return '\n\r'.join(message)
    ...
    def _validate_pass_reset(self):
        ...
        for rec_id in self:
            ...
            if write_date + delta > datetime.now():
                raise PassError(
                    _('Passwords can only be reset every %d hour(s). Please contact an administrator for assistance.') % pass_min)
        return True
    ...
    def _check_password_history(self, password):
        ...
        for rec_id in self:
            ...
            if recent_passes.filtered(lambda r: crypt.verify(password, r.password_crypt)):
                raise PassError(
                    _('Cannot use the most recent %d passwords') % rec_id.company_id.password_history)
